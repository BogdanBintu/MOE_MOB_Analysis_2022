...
def get_local_max(im_dif,th_fit,delta=2):
...
class GaussianFit():
    def __init__(self,im,X,center=None,n_aprox=10,min_w=0.5,max_w=4.,delta_center=3.):
        ...
        if center is None:
            #h__ = self.im-np.min(self.im)
            #center = np.sum(h__*X,-1)/np.sum(h__)
            center=X[:,np.argmax(im)]
    ...
    def fit(self,maxfev=0,eps_frac=10E-3,eps_dist=10E-3,eps_angle=10E-3):
        """
        ...
        if len(self.p_)>len(self.im):
            ...
            parms0 = self.p_
            self.p_old = None
            parmsf,_ = leastsq(self.calc_eps,parms0,Dfun=self.calc_jac,maxfev=maxfev)
...
def fast_fit_big_image(im,centers_zxy,radius_fit = 4,avoid_neigbors=True,recenter=False ,verbose = True,better_fit=False,troubleshoot=False):
    ...
    if len(centers_zxy)>0:
        ...
        zb,xb,yb = np.reshape(np.indices([radius_fit*2+1]*3)-radius_fit,[3,-1]).astype(int)
...
def calc_inverse_m(ms):
    zxy_ = []
    for x_ in range(0,2048,2048/8):
        for y_ in range(0,2048,2048/8):
            for z_ in range(0,100,10):
                zxy_.append([z_,x_,y_])
    zxy_ = np.array(zxy_)
    zxy__ = zxy_.copy()
    for m in ms:
        zxy__ = apply_colorcor(zxy__,m)
    return calc_color_matrix(zxy_,zxy__)
def calc_color_matrix(x,y,order=2,no_z=False,ransac=False):
    ...
    for p in range(order+1):
        for i in range(1 if no_z else p+1):
            for j in range(p+1):
                if i+j<=p:
    ...
    m_=np.array(m_)
    return m_
def apply_colorcor(x,m=None,no_z=False):
